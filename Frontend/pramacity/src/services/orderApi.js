...
export async function createOrder(orderData) {
  try {
    // Block admin or employee users from creating orders on client-side
    try {
      const profile = localStorage.getItem("user_profile");
      const user = profile ? JSON.parse(profile) : null;
      if (user && (user.role === "admin" || user.role === "employee")) {
        throw new Error("Admin/Employee không được phép thanh toán/đặt hàng");
      }
    } catch (e) {
      if (e.message && e.message.includes("Admin/Employee không được phép")) {
        throw e;
      }
    ...
    if (idString.startsWith("0-")) {
      console.error(
        "❌ orderApi.createOrder - Rejected temporary ID (starts with 0-):",
        orderData.address_id
      );
      throw new Error(
        `Địa chỉ giao hàng không hợp lệ: ID tạm thời không được phép`
      );
    }
    ...
    if (cleanAddressId.length > 1 && cleanAddressId.startsWith("0")) {
      console.error(
        "❌ orderApi.createOrder - Rejected ID starting with 0:",
        orderData.address_id,
        "cleaned:",
        cleanAddressId
      );
      throw new Error(
        `Địa chỉ giao hàng không hợp lệ: ID bắt đầu bằng 0 không được phép`
      );
    }
    ...
    if (
      isNaN(addressIdInt) ||
      addressIdInt <= 0 ||
      addressIdInt.toString().charAt(0) === "0"
    ) {
      ...
      throw new Error(
        `Địa chỉ giao hàng không hợp lệ: ${orderData.address_id}`
      );
    }
    ...
    console.error(
      "❌ orderApi.createOrder - Error response:",
      error.response?.data
    );
    const message =
...
export async function getUserOrders() {
  try {
    ...
    if (response.data.success) {
      ...
      console.log(
        "✅ orderApi.getUserOrders - Returning orders:",
        orders.length
      );
      if (orders.length > 0) {
      ...
      throw new Error(
        response.data.message || "Lỗi khi lấy danh sách đơn hàng"
      );
    }
  } catch (error) {
...
export async function cancelOrder(orderId, reason = "") {
  try {
    ...
    if (idString.startsWith("0-")) {
      console.error(
        "❌ orderApi.cancelOrder - Rejected temporary ID (starts with 0-):",
        orderId
      );
    ...
    if (cleanId.length > 1 && cleanId.startsWith("0")) {
      console.error(
        "❌ orderApi.cancelOrder - Rejected ID starting with 0:",
        orderId,
        "cleaned:",
        cleanId
      );
      throw new Error(
        `ID đơn hàng không hợp lệ: ID bắt đầu bằng 0 không được phép`
      );
    }
    ...
    if (
      isNaN(validatedOrderId) ||
      validatedOrderId <= 0 ||
      validatedOrderId.toString().charAt(0) === "0"
    ) {
    ...
    const response = await api.put(
      `/orders/${validatedOrderId}/status`,
      requestBody
    );
...
export async function getOrderById(orderId) {
  try {
    ...
    if (idString.startsWith("0-")) {
      console.error(
        "❌ orderApi.getOrderById - Rejected temporary ID (starts with 0-):",
        orderId
      );
      throw new Error(`ID đơn hàng không hợp lệ: ID tạm thời không được phép`);
    }
    ...
    if (cleanId.length > 1 && cleanId.startsWith("0")) {
      console.error(
        "❌ orderApi.getOrderById - Rejected ID starting with 0:",
        orderId,
        "cleaned:",
        cleanId
      );
    ...
    if (response.data.success) {
      ...
      throw new Error(
        response.data.message || "Lỗi khi lấy thông tin đơn hàng"
      );
    }
  } catch (error) {
    const message =
